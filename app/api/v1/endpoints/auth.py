...
TERMS_PRIVACY_MARKDOWN = """**Last Updated: July 20, 2026**

Welcome to **MedPass360**. Please carefully review the following Terms of Service, Privacy Policy, Medical Disclaimer, and End-User Agreement before proceeding to set up your personal health profile.

### 1. **Medical & Healthcare Disclaimer**
- **MedPass360 is not a healthcare provider**: The application and platform are strictly intended for personal health record management, emergency profile accessibility, and record organization.
- **No Substitute for Professional Diagnosis**: Content, suggestions, or profile displays within MedPass360 **do not constitute medical advice**, diagnosis, or treatment plan. Always consult a qualified medical professional for health-related concerns.
- **Emergency Situations**: In case of a medical emergency, call your local emergency services (e.g., 911 or emergency medical response) immediately.

### 2. **Privacy & Data Protection**
- **Data Ownership**: You retain **100% ownership** of all health information, vitals, allergies, conditions, medications, and personal profile data entered into MedPass360.
- **Encryption & Security**: All sensitive medical records, biometric setup tokens, and identity data are encrypted **at rest (AES-256)** and **in transit (TLS 1.3)**.
- **Zero Unauthorized Data Sharing**: We **never sell**, rent, or monetize your Personal Health Information (PHI) or Personally Identifiable Information (PII) to third parties or advertisers.

### 3. **Biometric & Access Controls**
- **Local Biometric Storage**: Biometric authentication (Fingerprint, Face Unlock, Touch ID) data remains stored **exclusively within your device's secure enclave**. MedPass360 servers store only randomized cryptographic verification hashes.
- **Account Credentials & PIN**: You are responsible for maintaining the confidentiality of your access PIN, device passcodes, and verification credentials.

### 4. **User Responsibilities & Conduct**
- **Accuracy of Health Data**: You agree to provide accurate and truthful health profile details (such as blood type, emergency contacts, and active allergies) to ensure safety in emergency scenarios.
- **Caregiver & Dependent Profiles**: When managing a profile for *"Someone I care for"*, you confirm that you possess lawful authority, consent, or guardianship to store and manage their health information.

### 5. **Terms of Use & License**
- **Non-Transferable License**: MedPass360 grants you a personal, non-exclusive, non-transferable, revocable license to use the app for personal, non-commercial health management.
- **Service Availability**: While we strive for 99.9% platform uptime, MedPass360 does not guarantee uninterrupted operation or zero latency during system maintenance window periods.

### 6. **Copyright & Intellectual Property Notice**
- **Copyright © 2026 MedPass360 Inc. All Rights Reserved.**
- All logos, design assets, software code, dynamic interfaces, and trademarks are the exclusive property of **MedPass360 Inc.** Unauthorized copying, redistribution, or reverse engineering is strictly prohibited.
"""


@router.get("/terms-privacy", response_model=ApiResponse)
async def get_terms_privacy():
    return ApiResponse(
        success=True,
        message="Terms and privacy policy content retrieved successfully.",
        data={
            "content": TERMS_PRIVACY_MARKDOWN,
            "format": "markdown"
        }
    )


@router.post("/accept-terms", response_model=ApiResponse)
async def accept_terms(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    updated_user = update_user_terms_agreement(db, current_user.uid, True)
    user_resp = UserResponse.model_validate(updated_user)
    return ApiResponse(
        success=True,
        message="Terms and privacy agreement accepted successfully.",
        data={
            "user": user_resp
        }
    )
